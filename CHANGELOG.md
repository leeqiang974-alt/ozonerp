## v18 (2026-09-11) — 款式图片设置弹窗：总图库按「原图 / 翻译图 / AI 图」分组展示 | 已实测
**需求**：总图库里 15 张图混排，无法区分哪些是 1688 原图、哪些是翻译图、哪些是 AI 生成图。
**改动**（frontend/listing-editor.js + listing-editor.html v=108）：
1. 新增 `kindOf(url)` 图片分类：URL 含 `/translated/` → 翻译图；含 `/generated/` → AI 图；其余（1688/阿里 CDN 等外部图）→ 原图。
2. 总图库 pane 按三类分组渲染：每组带小标题（如「原图（15）」）+ 该组图片网格。
3. 每张卡片标签由「公共图 · 可补充」改为「原图 · 可补充 / 翻译图 · 可补充 / AI 图 · 可补充」。
**验证**：node --check 通过；浏览器实测弹窗显示「原图（15）」分组与卡片类型标签；kindOf 单测四例通过（原图/翻译图/AI 图/本地图）。
**说明**：只新增分类展示维度，图库既有逻辑（设为首图 / 加入 / 删除 / 拖拽排序 / 点击放大）未动。
