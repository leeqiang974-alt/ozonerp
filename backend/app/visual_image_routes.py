...
class GenerateRequest(BaseModel):
    ...
    slots:list[Literal["hero","dimensions","details","steps","lifestyle"]]|None=None
class ApplyRequest(BaseModel):
    listing_draft_id:int=Field(gt=0); selected_urls:list[str]=Field(min_length=1,max_length=50); confirm_replace:bool=False
...
def generate(shop_id:int,payload:GenerateRequest,background_tasks:BackgroundTasks,db:Session=Depends(get_db)):
    try:
        ...
        job, should_start = queue_set(db,shop_id,payload.source_product_id,payload.listing_draft_id,requested_slots)
        if should_start:
            background_tasks.add_task(run_queued_set,shop_id,payload.source_product_id,payload.listing_draft_id,requested_slots)
...
def get_job(shop_id:int,source_product_id:int,db:Session=Depends(get_db)):
    return serialize(db.scalar(select(VisualImageJobRecord).where(VisualImageJobRecord.shop_id==shop_id,VisualImageJobRecord.source_product_id==source_product_id)))
...
def apply(shop_id:int,job_id:int,payload:ApplyRequest,db:Session=Depends(get_db)):
    ...
    try:return serialize(apply_set(db,job,draft,payload.selected_urls,"erp-local-operator"))
    except ValueError as exc:raise HTTPException(422,str(exc)) from exc
