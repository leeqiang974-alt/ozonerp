...
> 2026-07-24 G4-S1 可信佣金自动前置：修复本地售价已经生成、但强预检因 `manual_default` 佣金必然阻塞，而且旧一键链会先调用付费 AI 再暴露该问题。商品页现在在 AI 前通过 signed ERP session 自动分页读取当前店铺 `/v3/product/list`，分批用 `/v3/product/info/list` 核对精确类目并用官方 `/v5/product/info/prices` 的 `sales_percent_fbs` 读取 FBS 佣金；不再把 v3 `sale_schema=standard` 猜成 FBS。分页 total/cursor 矛盾、缺失/重复/额外 product ID、详情或价格覆盖不足、样本缺失/冲突以及 store/environment/source snapshot 变化全部 fail-closed。证据 hash、样本数、读取时间和完整范围绑定定价诊断，保存时再原子重验类目与快照；服务端实际模型调用入口也强制同一佣金/环境门，旧客户端和直接 API 不能绕过。`learned_product` 仍明确是估算，不冒充 Ozon 官方类目实时费率。定向与全量、lint、offline acceptance 通过；当前 ERP signed session 尚未建立，因此未执行真实 Seller API 读取、未调用付费 AI、未执行 Ozon 写入。G4-S1 仍为 `CURRENT`，下一入口是恢复同环境 signed session 后对当前商品运行一次只读佣金回放。

> 2026-07-24 G4-S1 卖家直选 capture 的内容与定价续跑修复：当前商品是卖家在 1688 明确采集并选中的真实商品，不应再依赖选品候选池的 `bestMatch`。现有一键链可直接使用精确绑定的 `candidateData + pricingPreview` 进入付费 AI 内容节点；付费回执输入哈希同时绑定定价预览，价格变化不能错误复用旧 AI 回执。Payload 在没有完整已验证 `bestMatch` 时只读取当前 `sourceEvidence.snapshotHash` 严格绑定的 SKU 价格，取当前快照最高 SKU 价进入定价；旧快照、缺失 variants evidence、无稳定 source SKU 或跨快照 pricing preview 均在 Payload/付费 AI 前阻断。独立审查发现并关闭原始 `variant.price` 回流和空 `bestMatch` 绕过后，最终复审结论 Ready。当前真实商品浏览器运行态显示 9 个 SKU 采集价 2.2–2.2 CNY、售价 22.45 CNY、50g / 10×10×10mm，且“自动完成其余资料”可用；利润仍因可信类目佣金/结算证据缺失保持未知。定向 382/382、全量 1330/1330、lint 76 files、offline acceptance 通过；未点击付费 AI、未执行 Seller API/Ozon 写入。G4-S1 仍为 `CURRENT`。

> 2026-07-24 G4-S1 付费 AI 结果幂等复用与提交版本栅栏：审计发现“一键自动完成”若已成功生成俄文文案、但随后 Payload 刷新或强预检失败，重试会再次进入付费内容节点。现在首次成功返回会保存 `paid_ai_content_v1` 回执，精确绑定 workflow/job/capture/store/source snapshot、生成输入哈希和内容哈希；同一绑定且内容未变时只复用文案、重建草稿并继续预检，`paidAiCalled=false`，商品输入、类目或内容任一变化都会使回执失效。并发生成由原子 lease 去重，模型请求强制在 lease 前超时，未知结果保留锁避免立即重扣；canonical 预检与提交均强制重验当前 job 和草稿回执，缺失/替换/过期全部 fail-closed。提交前 workflow reservation 与 job submission fence 同时冻结草稿、五字段绑定、输入和内容哈希，任何并发修改或锁建立异常都会在 Ozon 调用前停止并清理占位。确认文案同步说明“首次可能付费、未改动结果不会重复调用”。定向安全边界 339/339、全量 1327/1327、lint 76 files、offline acceptance 通过，独立复审结论 Ready；未调用付费 AI、未执行 Seller API/Ozon 写入。G4-S1 仍等待用户对当前真实商品首次生成的明确授权。
