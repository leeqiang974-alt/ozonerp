...
- Listing 质量诊断新增只读图片质量建议：
  ...
  - 本切片不写 Payload、不调用图片生成、不触发 GPT/Image 成本、不提交 Ozon；OCR/水印风险后续再从图片准备结果接入。
...
- 必填属性规则引擎 V2 继续补充尺码、包装/套装数量、适用场景候选：
  ...
  - `Сценарий использования` / 适用场景字段在商品文本含 travel/旅行、office/办公、bath/浴室、outdoor/户外等明确场景词时，只候选当前属性字典里的对应场景值。
...
- 继续做“必填属性规则引擎 V2”和媒体质量：把尺寸组合（如 `10x20cm`）、更多 Ozon 场景词、OCR/水印风险接入图片质量建议；同时把变体修复推进到整组差异建议。
