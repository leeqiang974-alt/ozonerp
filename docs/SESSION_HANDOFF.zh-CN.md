...
- `sync_states` 按店铺和资源保存最近成功时间、商品滚动游标、订单窗口及 5 分钟租约。同一资源 5 分钟内不重复访问 Ozon，多标签页同时触发只允许一个任务启动。
- FBS 订单采用上次成功窗口减 10 分钟的重叠校正；失败不推进检查点。类目缓存 24 小时；订单图片增加 24 小时校正时间并在订单行 upsert 时保留，避免每次订单同步后重新下载已有图片。
