...
- 货源文本含 `宠物` / `猫` / `狗` / 独立英文词 `pet` / `cat` / `dog` 时，只在当前类目当前属性字典中候选 `для животных` 等宠物用途值；`catalog` 这类包含 `cat` 片段的普通词不能触发。
...
- 货源文本含 `女士` / `女性` / 独立英文词 `women` / `female` / 明确俄文女性词形 `женский` / `женщина` 时，只在当前属性字典中候选 `женский` 等女性值；冒号、括号等标点后的 `женщина` 仍可识别，但 `еженедельник` 这类普通词不能因包含 `жен` 片段触发。
- 货源文本含 `男士` / `男性` / `men` / `male` / `муж` 时，只在当前属性字典中候选 `мужской` 等男性值。
- 货源文本含 `儿童` / 独立英文词 `kids` / 明确俄文儿童词形 `детский` 等线索时，只在当前属性字典中候选 `детский` 等儿童值；冒号、括号等标点后的 `детская` 仍可识别，但 `деталь` 这类普通词不能因包含 `дет` 片段触发。
