...
- 来源快照或绑定手工实测中的包装尺重必须按原值进入定价和 Payload；系统不得通过加缓冲、抬高最小值或截断来改写证据事实。
- 父级四项包装证据完整时，SKU 行中的孤立尺重不得覆盖父级值；Ozon 尺重拒绝也不得进入后台自动纠偏或自动重投。
- 如果 Ozon 拒绝当前尺重，系统必须停下并要求修复来源/实测证据，不能把尺重替换为“安全默认值”后自动重试。
